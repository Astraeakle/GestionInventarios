...
<template>
    <div>
        ...
        <VRow>
            <VCol cols="12">
                <VCard class="mb-6" title="Información General">
                    <VCardText>
                        <VRow>
                            <VCol cols="3">
                                <VTextField label="Solicitante" placeholder="" disabled v-model="user.full_name" />
                            </VCol>
                            <VCol cols="3">
                                <VTextField label="Sucursal" placeholder="" disabled v-model="user.sucursale.name" />
                            </VCol>
                            <VCol cols="3">
                                <VSelect v-model="warehouse_start_id" :items="warehouses" label="Almacen de Atención"
                                    placeholder="Select Almacen" item-title="name" item-value="id" eager
                                    :disabled="state >= 3" />
                            </VCol>
                            <VCol cols="3">
                                <VSelect v-model="warehouse_end_id" :items="warehouses" label="Almacen de Entrega"
                                    placeholder="Select Almacen" item-title="name" item-value="id" eager
                                    :disabled="state >= 3" />
                            </VCol>
                        </VRow>
                        <VRow>
                            <VCol cols="3">
                                <AppDateTimePicker label="Fecha de emisión" placeholder="Select date"
                                    v-model="date_emision" disabled />
                            </VCol>
                            <VCol cols="3">
                                <VTextarea label="Nota final" placeholder="" v-model="description" />
                            </VCol>
                            <VCol cols="3">
                                <VSelect v-model="state" :items="[
                                    {
                                        id: 1,
                                        name: 'Pendiente',
                                    },
                                ...
                                ]" label="Estado" placeholder="Select Almacen" item-title="name"
                                    item-value="id" eager />
                            </VCol>
                                ...
                                <VRow>
                                    <VCol cols="12">
                                        <VAutocomplete v-model="select_product" v-model:search="search_product"
                                            ...
                                            return-object placeholder="Search for a product" label="¿Que agregamos?"
                                        ...
                                        <VRow>
                                            <VCol cols="4">
                                                <VSelect :items="units" item-title="name" item-value="id"
                                                    placeholder="Select" v-model="unit_id" label="Unidades" />
                                            </VCol>
                                            <VCol cols="4">
                                                <VTextField label="Precio" type="number" v-model="price_unit"
                                                    placeholder="" />
                                            </VCol>
            ...
            <VCol cols="12">
                <VCard class="mb-6" title="Detalle de la S. Transporte">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(transport_detail, index) in transport_details" :key="index">
                                <td>
                                    ...
                                    <VBtn v-if="transport_detail.state == 1"
                                        @click="transportDetailSalida(transport_detail)" icon="ri-contract-line"
                                        variant="text" />

                                    <!-- DAR ENTREGA A LOS PRODUCTOS -->
                                    <VBtn v-if="transport_detail.state == 2"
                                        @click="transportDetailEntrega(transport_detail)" icon="ri-file-list-3-line"
                                        variant="text" />
                                </td>
                                <td>
                                ...
                                <td>
                                    <IconBtn size="small" @click="editTransportDetail(transport_detail)">
                                        <VIcon icon="ri-pencil-line" />
                                    </IconBtn>
                                    <IconBtn size="small" @click="deleteTransportDetail(transport_detail)">
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td>
                                    <VTextField label="Importe" placeholder="" type="number" v-model="importe" />
                                </td>
                                <td>
                                    <VTextField label="Igv" placeholder="" type="number" v-model="igv" />
                                </td>
                                <td>
                                    <VTextField label="Total" placeholder="" type="number" v-model="total" />
                                </td>
                                <td>
        ...
        <TransportDetailEditDialog v-if="transport_detail_selected && isTransportDetailEditDialogVisible"
            v-model:isDialogVisible="isTransportDetailEditDialogVisible"
            :transportDetailSelected="transport_detail_selected" :transport_id="route.params.id"
            :warehouse_start_id="warehouse_start_id" @editTransportDetail="transportDetailEdit">
        </TransportDetailEditDialog>

        <TransportDetailDeleteDialog v-if="transport_detail_delete_selected && isTransportDetailDeleteDialogVisible"
            v-model:isDialogVisible="isTransportDetailDeleteDialogVisible"
            :transportDetailSelected="transport_detail_delete_selected" @deleteTransportDetail="transportDetailDelete">
